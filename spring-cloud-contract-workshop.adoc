...
* Verify that the test succeeds and the stibs work as expected
...
== Exercise 7: Using @EnableStubRunnerServer
...
* The application requires this dependency: +
<dependency>
...
* You can use `scripts/order-coffee.sh` for this.

e.g. with stubrunner
* Discover StubRunner Boot -> provide stubs and messages `java -jar stubrunner.jar --stubrunner.stubsMode="LOCAL" --stubrunner.ids="groupId:artifactId:version:stubs:9876 --server.port:8083` +
using  `@EnableStubRunnerServer`
